...
export default function StudentsPage({ students, setStudents, att, toast }) {
  ...
  function add() {
    ...
    if (students.find((s) => s.username === form.username)) {
      setFormErr("Bu username band!"); return;
    }
  ...
  return (
    <div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>Login</th><th>Davomat</th><th>Holat</th><th>Amal</th>
              </tr>
            </thead>
            <tbody>
              {shown.length === 0 ? (
                <tr><td colSpan="7"><div className="empty"><div className="empty-ic">🔍</div>Topilmadi</div></td></tr>
              ) : (
                shown.map((s, i) => {
                    ...
                    <tr key={s.id}>
                      ...
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                          <div className="av av-s" style={{ width: 30, height: 30, borderRadius: 7, fontSize: 11 }}>{ini(s.name)}</div>
                          <div>
                      ...
                      <td style={{ fontFamily: "Fira Code", fontSize: 11.5, color: "var(--t3)" }}>{s.username}</td>
                      <td style={{ minWidth: 110 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                          <div className="pb" style={{ flex: 1 }}>
                            <div className={`pf ${pClass(st.p)}`} style={{ width: `${st.p}%` }} />
                          </div>
                          <span style={{ fontSize: 11, fontFamily: "Fira Code", fontWeight: 700, color: pColor(st.p) }}>{st.p}%</span>
                        </div>
                      </td>
                      <td>
                      ...
                      <td>
                        {s.status === "active" ? (
                          <button className="btn btn-sm btn-rd" onClick={() => setShowDel(s)}>🗑 O'chirish</button>
                        ) : (
                          <button className="btn btn-sm btn-gn" onClick={() => reactivate(s.id)}>✅ Faollashtirish</button>
                        )}
                      </td>
                    </tr>
                  );
      ...
      {/* Add modal */}
      {showAdd && (
        <Modal title="➕ Yangi o'quvchi qo'shish" sub="Ma'lumotlarni to'ldiring" onClose={() => setShowAdd(false)}>
      ...
      {/* Delete modal */}
      {showDel && (
        <Modal title="⚠️ O'quvchini o'chirish" sub={`${showDel.name} ni o'chirishni tasdiqlaysizmi?`} onClose={() => setShowDel(null)}>
          <p style={{ color: "var(--t3)", fontSize: 13 }}>O'quvchi tizimga kira olmaydi. Keyinchalik qayta faollashtirish mumkin.</p>
          <div className="mrow">
